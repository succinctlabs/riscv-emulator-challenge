...
impl Syscall for HintLenSyscall {
    fn execute(
        ...
        if ctx.rt.state.input_stream_ptr >= ctx.rt.state.input_stream.len() {
            return Err(ExecutionError::InvalidMemoryAccess(Opcode::UNIMP, ctx.rt.state.input_stream_ptr));
        }
use super::{Syscall, SyscallCode, SyscallContext};
...
impl Syscall for HintReadSyscall {
    fn execute(&self, ctx: &mut SyscallContext, _: SyscallCode, ptr: u32, len: u32) -> Result<Option<u32>, ExecutionError> {
        if ctx.rt.state.input_stream_ptr >= ctx.rt.state.input_stream.len() {
            return Err(ExecutionError::InvalidMemoryAccess(Opcode::UNIMP, ctx.rt.state.input_stream_ptr));
        }
        &self,
